...
CRenderPrototypeSdkRender::CRenderPrototypeSdkRender(
...
CRhinoSdkRender::RenderReturnCodes CRenderPrototypeSdkRender::Render(const ON_2iSize& sizeRender)
{
	if (!::RhRdkIsAvailable())
...
CRhinoSdkRender::RenderReturnCodes CRenderPrototypeSdkRender::RenderWindow(CRhinoView* pView, const LPRECT pRect, bool bInPopupWindow)
{
	if (!::RhRdkIsAvailable())
		return CRhinoSdkRender::render_error_starting_render;
...
BOOL CRenderPrototypeSdkRender::NeedToProcessGeometryTable()
{
	return ::RenderPrototypePlugIn().SceneChanged();
}

BOOL CRenderPrototypeSdkRender::NeedToProcessLightTable()
{
	return ::RenderPrototypePlugIn().LightingChanged();
}

BOOL CRenderPrototypeSdkRender::RenderPreCreateWindow()
{
	::RenderPrototypePlugIn().SetSceneChanged(FALSE);
	::RenderPrototypePlugIn().SetLightingChanged(FALSE);

	return TRUE;
}

BOOL CRenderPrototypeSdkRender::RenderContinueModal()
{
	return m_bContinueModal;
}

void CRenderPrototypeSdkRender::SetContinueModal(bool b)
{
	m_bContinueModal = b;
}

void CRenderPrototypeSdkRender::RenderThread(void* pv) // Static.
{
	reinterpret_cast<CRenderPrototypeSdkRender*>(pv)->ThreadedRender();
}

void CRenderPrototypeSdkRender::StartRendering()
{
	m_hRenderThread = ::CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)RenderThread, this, 0, NULL);
}

BOOL CRenderPrototypeSdkRender::StartRenderingInWindow(CRhinoView*, const LPCRECT)
{
	m_hRenderThread = ::CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)RenderThread, this, 0, NULL);
...
void CRenderPrototypeSdkRender::StopRendering()
{
...
//我要处理的渲染函数
int CRenderPrototypeSdkRender::ThreadedRender(void)
{
